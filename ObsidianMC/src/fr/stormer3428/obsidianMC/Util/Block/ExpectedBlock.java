...
public class ExpectedBlock {

	private final Location location;
	private final Material displayMaterial;
	private final ArrayList<Material> acceptedMaterials = new ArrayList<>();
	
	public ExpectedBlock(Location location, Material displayMaterial, Material ... acceptedMaterials) {
		this.location = location.add(0.5,0.5,0.5);
		this.displayMaterial = displayMaterial;
		this.acceptedMaterials.addAll(Arrays.asList(acceptedMaterials));
	}
	
	public ExpectedBlock(Location location, Material displayMaterial, ArrayList<Material> acceptedMaterials) {
		this.location = location.add(0.5,0.5,0.5);
		this.displayMaterial = displayMaterial;
	...
	public boolean isValid() {
		...
		TemporaryBlock.createNew(displayMaterial == null ? Material.RED_STAINED_GLASS : Material.BARRIER, 20 * 4, location).getRemainingTicks();
		return false;
	}
